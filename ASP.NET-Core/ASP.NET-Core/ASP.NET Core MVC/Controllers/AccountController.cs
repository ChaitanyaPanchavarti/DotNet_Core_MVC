...
namespace ASP.NET_Core_MVC.Controllers
{
    public class AccountController : Controller
    {
        ...
        public IEmailService em { get; set; }
                                                                ...
                                                                string remoteError = null)
        {
            ...
            ViewBag.ErrorMessage = $"Please contact support on panchavarthichaitanya@gmail.com";
            return View("Error");
        }
        [HttpGet]
        [AllowAnonymous]
        ...
        public async Task<IActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    string link=await GenerateAndLogConfirmationLink(user);

                    if (signInManager.IsSignedIn(User) && User.IsInRole("Admin"))
                    {
                        return RedirectToAction("ListUsers", "Administration");
                    }
                    // await SendEmail(model.Email,link);
                   // return RedirectToAction("SentEmail", "EmailConfirmation", (model.Email, link));
        ...
        private async Task SendEmail(string email, string link)
        {
            ...
            string confirmationLink = link;
            await em.SendEmailAsync(email, "Confirm your email", $"Please confirm your email by clicking <a href='{confirmationLink}'>here</a>.");
        ...
        // PRIVATE METHODS
        private async Task<string> GenerateAndLogConfirmationLink(ApplicationUser user)
        {
           //var email= await userManager.FindByIdAsync(user);
